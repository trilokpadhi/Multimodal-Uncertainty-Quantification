...
│   │               ├── generated_token_ids: Generated token IDs.
│   │               ├── decoded_outputs: Decoced generated tokens. 
│   |               ├── grounding_score: Grounding scores for the explanation ## if the scores arent generated then there will not be any grounding score, which could be due to the model not being able to generate the scores due to some error which can be checked in the key 'error_grounding' in the response.
│   |               ├── image: image path
