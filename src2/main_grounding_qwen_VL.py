...
from inference_utils import generate_grounded_segmentation, generate_grounding_with_llama3_2, generate_grounding_with_qwen_vl
...
from transformers import pipeline, AutoProcessor, Qwen2VLForConditionalGeneration, Qwen2_5_VLForConditionalGeneration
import pickle
...
def grounding_inference_pipeline(config, rank, world_size):
    ...
    # model_id = config['grounding']['qwen_vl_model_path']
    model_id = config['grounding']['qwen_vl_25_7B_model_path']
    ...
    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_id, torch_dtype=torch.float16).to(device)
    ...
    all_files = [f for f in all_files if os.path.isfile(os.path.join(config['logging']['explanation_dir'], f))]
    ...
    for file in tqdm(subset_files, desc=f"Rank {rank} processing"):
        ...
        #     config_logging=config['logging']
        # )
        generate_grounding_with_qwen_vl(
            file=file,
            model=model,
            processor=processor,
            rank=rank,
