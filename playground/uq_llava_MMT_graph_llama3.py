...
def construct_graph_llama3(data):
    """
    Construct a graph from the output of the llama3 model.
    """
    ...
    # Add attributes as edges connecting the entity to the attribute value
    for attribute in data["attributes"]:
        entity = attribute["entity"]
        attribute_value = attribute["value"]
        G.add_node(attribute_value, type="attribute_value")
        G.add_edge(entity, attribute_value, description=attribute["type"])

    # Add relations as edges with description as labels
    for relation in data["relations"]:
        G.add_edge(relation["entity1"], relation["entity2"], description=relation["description"])
    ...
    for idx, result in tqdm(args.responses.items(), desc='Calculating uncertainty'):
        ...
        for i, sentence in enumerate(all_sentences):
            try:
                ...
                llama3_graph = extract_entities_and_relationships_llama3(sentence, args)
                json_graph = extract_json_from_text(llama3_graph)
                G = construct_graph_llama3(json_graph)
                graphs.append(G)
                
                if i == 0:  # Ground truth graph
                    result['ground_truth_graph_llama3'] = graph_to_adj_list(G)
                else:
                    responses[i - 1]['graph_llama3'] = graph_to_adj_list(G)
            except Exception as e:
                print(f"Error constructing graph: {e}")
                if i > 0:
                    responses[i - 1]['graph_llama3'] = None
