...
#     plt.plot(bin_centers, avg_accuracies, marker='o', label=f'Reliability ({confidence_type})')
...
#     bins = np.linspace(0, 1, num_bins + 1)  # Create equal-width bins
#     bin_indices = np.digitize(confidences, bins) - 1  # Bin indices for each confidence score
...
#             avg_accuracy = np.mean(accuracies[bin_mask])
#             bin_ece = (bin_size / total_samples) * np.abs(avg_confidence - avg_accuracy)
#             ece += bin_ece
#     return ece

# def plot_reliability_diagram(confidences, accuracies, confidence_type, save_path, num_bins=10):
#     bins = np.linspace(0, 1, num_bins)  # Create bins between 0 and 1
#     bin_indices = np.digitize(confidences, bins) - 1
...
#             gap_values.append(avg_confidence - avg_accuracy)
#             bin_centers.append((bins[i] + bins[i + 1]) / 2)  # Get bin center
#         else:
#             # avg_accuracies.append(0)
#             # avg_confidences.append(0)
#             # gap_values.append(0)
#             # bin_centers.append((bins[i] + bins[i + 1]) / 2)
#             print(' no samples in the bin ')
#             continue
...
#     ece = calculate_ece(confidences, accuracies, num_bins=num_bins)

#     plt.figure(figsize=(8, 6))  # Set figure size

#     # Plot the "Outputs" as a solid blue bar
#     plt.bar(bin_centers, avg_accuracies, width=bin_width, color='blue', edgecolor='black', alpha=0.8, label='Outputs')

#     # Plot the "Gap" between expected and actual accuracy as a hatched bar
#     plt.bar(bin_centers, gap_values, width=bin_width, bottom=avg_accuracies, color='red', edgecolor='red', hatch='//', alpha=0.3, label='Gap')

#     # Add the perfect calibration line
#     plt.plot([0, 1], [0, 1], linestyle='--', color='gray', label='Perfect Calibration', linewidth=2, alpha=0.7)

#     # Customize labels and title
#     plt.xlabel('Confidence', fontsize=14)
#     plt.ylabel('Accuracy', fontsize=14)
#     plt.title(f'Reliability Diagram ({confidence_type})', fontsize=16)

#     # Increase font sizes and customize legend
#     plt.xticks(fontsize=12)
#     plt.yticks(fontsize=12)
#     plt.legend(fontsize=12, loc='upper left')

#     # Customize the grid
#     plt.grid(True, which='both', linestyle=':', linewidth=1.5, alpha=0.6)

#     # Display the ECE value
#     plt.text(0.7, 0.2, f'ECE={ece:.2f}', fontsize=14, bbox=dict(facecolor='lightblue', alpha=0.5))

#     # Save and display the plot
#     plt.tight_layout()  # Adjust layout to ensure everything fits well
#     plt.savefig(save_path, dpi=300)  # Increase resolution for better quality
#     plt.show()
#     plt.clf()  # Clear the figure to avoid overlap with future plots
